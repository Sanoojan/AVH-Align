...
def labels_to_segments(labels):
    return binary_segments(np.asarray(labels).astype(int).reshape(-1))
...
def compute_localization_ap(framewise_labels, proposal_segments, iou_thresholds=(0.5, 0.75, 0.95)):
    gt_segments_by_video = [labels_to_segments(labels) for labels in framewise_labels]
    total_gt = sum(len(segments) for segments in gt_segments_by_video)
    proposals_by_video = [
        prepare_proposals(proposals, label_length=len(labels))
        for proposals, labels in zip(proposal_segments, framewise_labels)
    ]
    metrics = {}

    for iou_threshold in iou_thresholds:
        ranked_predictions = []
        for video_idx, proposals in enumerate(proposals_by_video):
            for proposal_idx, (score, start, end) in enumerate(proposals):
                ranked_predictions.append((float(score), video_idx, proposal_idx, (float(start), float(end))))

        if total_gt == 0 or not ranked_predictions:
            metrics[iou_threshold] = 0.0
            continue

        ranked_predictions.sort(key=lambda item: (-item[0], item[1], item[2]))
        matched_gt = [set() for _ in gt_segments_by_video]
        true_positives = np.zeros(len(ranked_predictions), dtype=float)
        false_positives = np.zeros(len(ranked_predictions), dtype=float)

        for pred_idx, (_, video_idx, _, pred_segment) in enumerate(ranked_predictions):
            best_gt_idx = None
            best_iou = 0.0
            for gt_idx, gt_segment in enumerate(gt_segments_by_video[video_idx]):
                if gt_idx in matched_gt[video_idx]:
                    continue
                iou = segment_iou(pred_segment, gt_segment)
                if iou > best_iou:
                    best_iou = iou
            ...
            if best_gt_idx is not None and best_iou >= iou_threshold:
                matched_gt[video_idx].add(best_gt_idx)
        ...
        metrics[iou_threshold] = float(
            np.sum((recall[change_points + 1] - recall[change_points]) * precision[change_points + 1])
        )

    return metrics


def compute_localization_ar(
    framewise_labels,
    proposal_segments,
    proposal_counts=(100, 50, 20, 10),
    iou_thresholds=None,
):
    ...
    for proposal_count in proposal_counts:
        ...
        for iou_threshold in iou_thresholds:
            true_positives = 0
            for proposals, gt_segments in zip(proposals_by_video, gt_segments_by_video):
                top_proposals = proposals[:proposal_count]
                for gt_segment in gt_segments:
                    if any(segment_iou((proposal[1], proposal[2]), gt_segment) >= iou_threshold for proposal in top_proposals):
                        true_positives += 1
            recalls.append(true_positives / total_gt if total_gt > 0 else 0.0)
...
def print_localization_metrics(title, ap_metrics, ar_metrics):
    print(f"\n{title}")
    print("=" * len(title))
...
def extract_framewise_scores(predictions, valid_length, factor):
    finest_idx = min(range(len(factor)), key=lambda idx: factor[idx])
    scores = torch.sigmoid(predictions[finest_idx][0, :, 0]).detach().cpu().numpy().astype(float)
    positions = np.arange(len(scores), dtype=float) * float(factor[finest_idx])

    if valid_length <= 0:
        return np.array([], dtype=float)
    if len(scores) == valid_length:
        return scores
    if len(scores) == 0:
        return np.zeros(valid_length, dtype=float)
    if len(scores) == 1:
        return np.full(valid_length, scores[0], dtype=float)

    target_positions = np.arange(valid_length, dtype=float)
    return np.interp(target_positions, positions, scores, left=scores[0], right=scores[-1])
...
def process_video(feature_path, model, max_length, factor, device, metrics_device, feature_dim):
    ...
    frame_scores = extract_framewise_scores(predictions, valid_length, factor)
    ...
    return video_score, frame_scores, proposals, valid_length
...
def main(args):
    ...
    for _, row in tqdm(metadata.iterrows(), total=len(metadata)):
        ...
        score, frame_scores, proposals, valid_length = process_video(feature_path, model, max_length, factor, device, metrics_device, feature_dim)
        ...
        frame_scores = np.asarray(frame_scores, dtype=float).reshape(-1)[: len(labels)]

        outputs.append(score)
        ground_truths.append(label)
        framewise_scores.append(frame_scores)
    ...
    if args.plot_framewise:
        plot_framewise_predictions(
            ...
            ranking_threshold=args.framewise_confidence_threshold,
        )
    ...
    localization_ap = compute_localization_ap(
        framewise_labels,
        proposal_segments,
        iou_thresholds=tuple(args.iou_thresholds),
    )
    localization_ar = compute_localization_ar(
        framewise_labels,
        proposal_segments,
        proposal_counts=tuple(args.ar_proposal_counts),
    ...
    print_localization_metrics("FRAMEWISE LOCALIZATION METRICS", localization_ap, localization_ar)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--framewise_confidence_threshold", type=float, default=0.5)
    parser.add_argument("--proposal_confidence_threshold", type=float, default=0.5, help="Deprecated; proposals are saved but framewise AP/AR uses --framewise_confidence_threshold.")
    parser.add_argument("--iou_thresholds", type=float, nargs="+", default=[0.5, 0.75, 0.95], help="IoU thresholds for localization AP.")
    parser.add_argument("--ar_proposal_counts", type=int, nargs="+", default=[100, 50, 20, 10], help="Top-N proposal counts for localization AR.")
    parser.add_argument("--ar_iou_thresholds", type=float, nargs="+", default=[0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8, 0.85, 0.9, 0.95], help="IoU thresholds averaged for localization AR.")
