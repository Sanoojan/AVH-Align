...
from model import ConvBoundaryTemporalFusionAV_only, SimpleTemporalFusion, SimpleTemporalFusionAV_only, TemporalFusionModel,BoundaryAwareAVLocalizer
...
def run_epoch(dataloader, model, device, loss_config, metric_config, optimizer=None, use_tqdm=False, compute_test_style_metrics=False):
    ...
    with torch.set_grad_enabled(is_training):
        for features, labels in loader:
            ...
            coarse_frame_bce = logits.new_tensor(0.0)
            boundary_bce = logits.new_tensor(0.0)
            boundary_offset = logits.new_tensor(0.0)

            if isinstance(model_output, dict):
                if loss_config.get("coarse_frame_bce_weight", 0.0) > 0 and "coarse_logits" in model_output:
                    coarse_frame_bce = frame_criterion(model_output["coarse_logits"], labels)
                ...
                if needs_boundary_targets:
                    ...
                    if boundary_weight > 0 and "boundary_logits" in model_output:
                        boundary_bce = boundary_criterion(model_output["boundary_logits"], boundary_targets)
                    if offset_weight > 0 and "offsets" in model_output:
                        boundary_offset = masked_smooth_l1_loss(model_output["offsets"], offset_targets, offset_mask)
