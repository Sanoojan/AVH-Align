...
from model import (
    AuvireAVDeepfake1MLocalizer,
    BoundaryAwareAVLocalizer,
...
from helpers.model_helpers import boundary_proposals as make_boundary_proposals
...
def squeeze_single_video_logits(logits):
    if logits.ndim == 2 and logits.shape[0] == 1:
        return logits[0]
    return logits.squeeze()


def extract_boundary_outputs(model_output):
    if not isinstance(model_output, dict):
        return None

    if "boundary_logits" in model_output:
        ...
        start_logits = squeeze_single_video_logits(model_output["start_logits"]).reshape(-1)
        end_logits = squeeze_single_video_logits(model_output["end_logits"]).reshape(-1)
        keep = min(start_logits.shape[0], end_logits.shape[0])
        boundary_logits = torch.stack((start_logits[:keep], end_logits[:keep]), dim=-1)
    else:
        return None

    outputs = {"boundary_logits": boundary_logits.detach().cpu().numpy()}
    if "offsets" in model_output:
        offsets = model_output["offsets"]
        if offsets.ndim == 3 and offsets.shape[0] == 1:
            offsets = offsets[0]
        else:
            offsets = offsets.squeeze()
        outputs["offsets"] = offsets.detach().cpu().numpy()
    return outputs
...
def transform_auvire_predictions(predictions, max_length, factor, metrics_device):
    ...
    first_indices = torch.arange(predictions.shape[0], device=metrics_device)[:, None]
    predictions = predictions[first_indices, indexes]
    return soft_nms_torch_parallel(predictions, sigma, t1, t2, fps, metrics_device)


def extract_auvire_proposals(model_output, factor, metrics_device):
    predictions = model_output["auvire_outputs"]
    max_length = predictions[0].shape[1]
    ...
    proposals = np.asarray(proposals, dtype=float).reshape(-1, 3)
...
def process_video(data, fusion_model, device):
    ...
    if args.model_name == "FusionModel":
        ...
        if args.model_name == "AuvireAVDeepfake1MLocalizer" and isinstance(model_output, dict) and "auvire_outputs" in model_output:
            proposals = extract_auvire_proposals(
                model_output,
                factor=fusion_model.factor,
                metrics_device=torch.device(args.metrics_device),
            )
            frame_scores = torch.from_numpy(rasterize_proposals(proposals, stacked_feat.shape[2])).to(device)
            score = float(proposals[:, 0].max()) if len(proposals) else 0.0
            return score, frame_scores.reshape(-1), {"proposals": proposals}

        boundary_outputs = extract_boundary_outputs(model_output)
    ...
    return score, frame_scores.reshape(-1), boundary_outputs
...
def confidence_scores_for_boundary(frame_scores):
    frame_scores = np.asarray(frame_scores, dtype=float).reshape(-1)
    finite_scores = frame_scores[np.isfinite(frame_scores)]
    if finite_scores.size == 0:
        return np.zeros_like(frame_scores, dtype=float)
    if np.min(finite_scores) < 0.0 or np.max(finite_scores) > 1.0:
        return sigmoid_np(frame_scores)
    return np.nan_to_num(frame_scores, nan=0.0, posinf=1.0, neginf=0.0)
...
def boundary_outputs_to_proposals(frame_scores, boundary_outputs, label_length=None, max_proposals=None):
    if not boundary_outputs or "boundary_logits" not in boundary_outputs:
        return np.empty((0, 3), dtype=float)
    if max_proposals is None:
        max_proposals = max(SEGMENT_AR_PROPOSAL_COUNTS)
    ...
    frame_scores = np.asarray(frame_scores, dtype=float).reshape(-1)
    if label_length is not None:
        frame_scores = frame_scores[:label_length]
    label_length = len(frame_scores) if label_length is None else min(int(label_length), len(frame_scores))
    if label_length <= 0:
        return np.empty((0, 3), dtype=float)

    boundary_logits = np.asarray(boundary_outputs["boundary_logits"], dtype=float).reshape(-1, 2)[:label_length]
    if len(boundary_logits) == 0:
        return np.empty((0, 3), dtype=float)

    boundary_probs = sigmoid_np(boundary_logits)
    start_scores = boundary_probs[:, 0]
    end_scores = boundary_probs[:, 1]
    frame_confidences = confidence_scores_for_boundary(frame_scores)[:label_length]
    candidate_count = min(label_length, max(20, max_proposals * 4))

    candidates = make_boundary_proposals(
        frame_confidences,
        start_scores,
        end_scores,
        min_len=1,
        max_len=label_length,
        topk_starts=candidate_count,
        topk_ends=candidate_count,
        max_segments=max_proposals,
    ).tolist()

    offsets = boundary_outputs.get("offsets")
    if offsets is not None:
        offsets = np.asarray(offsets, dtype=float).reshape(-1, 2)[:label_length]
        frame_indices = np.argsort(-frame_confidences, kind="mergesort")[:candidate_count]
        for frame_idx in frame_indices:
            left_offset, right_offset = offsets[frame_idx]
            if not np.isfinite(left_offset) or not np.isfinite(right_offset):
                continue
            start = int(np.floor(frame_idx - max(0.0, left_offset)))
            end = int(np.ceil(frame_idx + max(0.0, right_offset) + 1.0))
            start = max(0, min(start, label_length))
            end = max(0, min(end, label_length))
            if end <= start:
                continue
            start_boundary = start_scores[start]
            end_boundary = end_scores[max(start, end - 1)]
            boundary_confidence = float(np.sqrt(start_boundary * end_boundary))
            confidence = 0.5 * boundary_confidence + 0.5 * float(frame_confidences[frame_idx])
            candidates.append([confidence, start, end])

    proposals = prepare_proposals(candidates, label_length=label_length)
    if proposals.size == 0:
        return proposals
    proposals = temporal_soft_nms(proposals, max_proposals=max_proposals)
    return prepare_proposals(proposals, label_length=label_length)[:max_proposals]
...
def build_segment_proposals(frame_scores, boundary_outputs=None, label_length=None, max_proposals=None):
    if max_proposals is None:
        max_proposals = max(SEGMENT_AR_PROPOSAL_COUNTS)

    if boundary_outputs and "proposals" in boundary_outputs:
        return prepare_proposals(boundary_outputs["proposals"], label_length=label_length)

    frame_proposals = frame_scores_to_proposals(
        frame_scores,
        label_length=label_length,
        max_proposals=max_proposals,
    )
    boundary_proposals = boundary_outputs_to_proposals(
        frame_scores,
        boundary_outputs,
        label_length=label_length,
        max_proposals=max_proposals,
    ...
    if boundary_proposals.size == 0:
        return frame_proposals
    if frame_proposals.size == 0:
        return boundary_proposals

    proposals = np.vstack([boundary_proposals, frame_proposals])
    proposals = temporal_soft_nms(prepare_proposals(proposals, label_length=label_length), max_proposals=max_proposals)
    return prepare_proposals(proposals, label_length=label_length)[:max_proposals]
...
def main(args):
    ...
    for _, row in tqdm(metadata.iterrows()):
        ...
        score,output,boundary_outputs = process_video(data, fusion_model, device)
        outputs.append(to_float(score))
        boundary_outputs_by_video.append(boundary_outputs)
        ground_truths.append(label)
        path_names.append(row["path"])
        
        if args.dataset == "FakeAVCeleb":
            if row["type"] == "RealVideo-RealAudio":
                labels = np.zeros(output.shape[0], dtype=int)
            else:
                labels = np.ones(output.shape[0], dtype=int)
        else:
            ...
            labels = data_labels["framewise_labels"]

        if args.model_name == "AuvireAVDeepfake1MLocalizer" and boundary_outputs and "proposals" in boundary_outputs:
            proposals = boundary_outputs["proposals"].copy()
            proposals[:, 1] = np.clip(proposals[:, 1], 0, len(labels))
            proposals[:, 2] = np.clip(proposals[:, 2], 0, len(labels))
            proposals = proposals[proposals[:, 2] > proposals[:, 1]]
            boundary_outputs["proposals"] = proposals
            framewise_scores.append(rasterize_proposals(proposals, len(labels)))
        else:
            framewise_scores.append(output.detach().cpu().numpy())
        framewis_labels.append(labels)
        # breakpoint()
        
    outputs = np.array(outputs)
    use_auvire_predictions = args.model_name == "AuvireAVDeepfake1MLocalizer"
    if use_auvire_predictions:
        framewise_confidence_threshold = 0.5
    elif args.frame_score_type == "sigmoid":
    ...
    if use_auvire_predictions:
        proposal_segments = [
            ...
            for scores, labels, boundary_outputs in zip(
                framewise_scores,
                framewis_labels,
                boundary_outputs_by_video,
            )
        ...
        proposal_segments = [
            ...
            for scores, labels, boundary_outputs, predicted_fake in zip(
                framewise_scores,
                framewis_labels,
                boundary_outputs_by_video,
                predicted_fake_videos,
            )
        ]
    ...
    frame_score_label = "auvire_proposal" if use_auvire_predictions else args.frame_score_type
    print(f"Framewise confidence threshold: {framewise_confidence_threshold:.1f} ({frame_score_label})")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_name", type=str, default="FusionModel",
                        help="Model architecture to use (FusionModel, TemporalFusionModel, SimpleTemporalFusion, SimpleTemporalFusionAV_only, ConvBoundaryTemporalFusionAV_only, BoundaryAwareAVLocalizer)")
