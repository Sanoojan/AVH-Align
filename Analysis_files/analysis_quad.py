...
checkpoint_path_ours = "checkpoints/AVH_A_V_simple_mlp_train_val_splits_video_wise_trimm_compare_500.pt"
...
features_root = "Features/AV1M-Trimmed/test"
labels_root   = "Features/Labels/test"
output_dir = "Plots/random_quadruplets_baseline_ours_500_frames"
...
df["type"] = df["path"].apply(lambda x: x.split("/")[-1].replace(".mp4", ""))

real_df = df[df["type"] == "real"]
fvra_df = df[df["type"] == "fake_video_real_audio"]
fvfa_df = df[df["type"] == "fake_video_fake_audio"]
rvfa_df = df[df["type"] == "real_video_fake_audio"]
...
for _ in range(num_quads):
    quad = {
        "real": real_df.sample(1).iloc[0]["path"],
        "fake_video_real_audio": fvra_df.sample(1).iloc[0]["path"],
        "fake_video_fake_audio": fvfa_df.sample(1).iloc[0]["path"],
        "real_video_fake_audio": rvfa_df.sample(1).iloc[0]["path"],
    }
...
for idx, quad in enumerate(quadruplets):
    ...
    for video_type, mp4_path in quad.items():
        # if video_type == "fake_video_real_audio":
        #     continue
        feature_path, label_path = build_paths(mp4_path)
    ...
    plt.subplot(2,2,1)
    plot_metric(plt.gca(), "audio_multimodal", "Cosine", metrics_dict, labels_dict)

    plt.subplot(2,2,2)
    plot_metric(plt.gca(), "ours", "Logits", metrics_dict, labels_dict)

    plt.subplot(2,2,3)
    plot_metric(plt.gca(), "audio_vs_visual", "Cosine", metrics_dict, labels_dict)
